...
FS.BotChatLoader = {
    ...
    onLoad: function (botConnectionName) {
        FS.BotChatLoader.botConnectionName = botConnectionName;
    ...
    setupBot: function() {
        if (FS.BotChatLoader.botConnectionName) {
            FS.BotChatLoader.authorizeConversation(FS.BotChatLoader.botConnectionName);
            return null;
        }
        else {
            var fetch = FS.BotChatLoader.getFetchForBot();
            return FS.Common.executeFetch(fetch, "Array", null)
                .then(FS.BotChatLoader.getBotConnectionName, FS.BotChatLoader.showErrorExecuteFetch)
    ...
    authorizeConversation: function (botConnectionName) {
        ...
        var params = {
            BotConnectionName: FS.BotChatLoader.botConnectionName
        };
    ...
    getBotConnectionName: function(result) {
    ...
    renderBotChat: function (conversation) {
        ...
        BotChat.App({
            ...
            bot: {
                id: FS.BotChatLoader.botConnectionName
            },
    ...
    displayError: function (message) {
        ...
        var errorElement = document.getElementById(FS.BotChatLoader.errorDivName);

        try {
            var messageJson = JSON.parse(message);
            errorElement.innerText = messageJson.responseText;
        }
        catch(e) {
            errorElement.innerText = FS.BotChatLoader.localization.get("Alert.BotAuthorizing.Failure");
        }
    },
